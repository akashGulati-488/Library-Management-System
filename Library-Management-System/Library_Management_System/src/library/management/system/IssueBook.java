...
public class IssueBook extends JFrame implements ActionListener{
    ...
    private JTextField book_id,book_name, book_publisher, book_edition, book_price, student_id, student_name, student_father_name, student_course, student_study_year, student_semester;
    private JButton issueSearch, studentSearch, issueBook, back;
	...
	public IssueBook() {
		...
		issueSearch = new JButton("Search");
		issueSearch.addActionListener(this);
		issueSearch.setBorder(new LineBorder(new Color(192, 192, 192), 1, true));
		issueSearch.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		issueSearch.setBackground(Color.BLACK);
		issueSearch.setForeground(Color.WHITE);
		issueSearch.setFont(new Font("Trebuchet MS", Font.BOLD, 14));
		issueSearch.setFocusable(false);
		issueSearch.setBounds(652, 59, 100, 30);
		mainPanel.add(issueSearch);
		...
		bookPanel.setBorder(new TitledBorder(new LineBorder(new Color(47, 79, 79), 2, true), "Issue Book",
					TitledBorder.LEADING, TitledBorder.TOP, null, new Color(34, 139, 34)));
		bookPanel.setFont(new Font("Tahoma", Font.BOLD, 14));
		bookPanel.setBounds(420, 38, 355, 238);
		...
		dateOfIssue.setForeground(new Color(105, 105, 105));
		dateOfIssue.setFont(new Font("Trebuchet MS", Font.BOLD, 15));
		dateOfIssue.setBounds(444, 340, 118, 26);
		mainPanel.add(dateOfIssue);

		chooseDate = new JDateChooser();
		chooseDate.setBorder(new LineBorder(new Color(0, 0, 0), 1, true));
		chooseDate.setForeground(new Color(105, 105, 105));
		chooseDate.setBounds(560, 337, 200, 29);
		...
		issueBook.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		issueBook.setBounds(640, 377, 118, 33);
		issueBook.setFocusable(false);
		issueBook.setBackground(Color.BLACK);
		issueBook.setForeground(Color.WHITE);
	    mainPanel.add(issueBook);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == issueBook) {
			try {
				...
				st.setString(12, ((JTextField) chooseDate.getDateEditor().getUiComponent()).getText());
				...
				if(res > 0) {
					...
					JOptionPane.showMessageDialog(null, "Error In Issuing The Book", "Error", JOptionPane.ERROR_MESSAGE);
				}
				st.close();
				con.conn.close();
			}
